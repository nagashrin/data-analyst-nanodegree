...
# Raw Data input
def get_raw_data (city_data):
    r = city_data.shape[0]
    i=1
    while (i <= r):
        i+=5
        more = input('\nDo you want to see 5 rows of raw data? Y/N: ')
        if more.lower() == 'y':
            print(city_data[0:i])
        else:
            break
...
def get_restart():
    restart = input('\nDo you want to explore another city? Y/N: ')
    if restart.lower() == 'y':
        main()
    else:
...
def show_data_points_non_visual(city_data):
    ...
    # Print Data Analysis
    print('\n* Most Popular Hour for your filter is: {}:00'.format(city_data['Start Time'].groupby(city_data['Start Time'].dt.hour).count().idxmax()))
    print('* Most Popular Day for your City is: {}'.format(city_data['Start Time'].groupby(city_data['Start Time'].dt.weekday_name).count().idxmax()))
...
def show_bikeshare_charts(city_data):
    print('\nPlease be patient, this will take upto 15 seconds...')
